...
- 포지션 관리 화면에 `포지션 조정` 작업을 추가했습니다.
- 분할상장, 주식분할, 티커 변경처럼 실제 거래가 아닌 변동을 종목명·보유 수량·평균 매수가·현재가에 직접 반영할 수 있습니다.
- 포지션 조정은 매수·매도 및 실현손익으로 집계되지 않습니다.
- 조정일과 사유, 조정 전후 값을 별도 감사 기록에 보관합니다.

## 이전 0.9.2 변경 사항

- 미국 동부시간 기준으로 가장 최근에 완료된 거래일을 판정합니다.
- Yahoo Finance 일봉에서 금요일 데이터가 누락되어도 이전 종가 메타데이터로 복구합니다.
- 보유 종목, SPY 및 섹터 대장주의 상대수익률에 동일한 완료 종가 규칙을 적용합니다.
- 숫자와 한글 글꼴을 Windows 기본 가변 글꼴 계열로 조정해 표시를 부드럽게 개선했습니다.
...
`Tuja-Setup-0.9.3-beta.exe`를 내려받아 실행합니다. Windows SmartScreen은 코드 서명이 없는 개인 베타 앱에 경고를 표시할 수 있습니다.
